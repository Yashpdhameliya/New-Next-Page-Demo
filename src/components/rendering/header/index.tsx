...
const Header = () => {
  ...
  const nonTransparentRoutes = ["/plans", "/auth", "/terms-and-conditions", "/waoclub"];

  const isTransparentBackground = !nonTransparentRoutes.some((route) => router.pathname.includes(route));
  ...
  return (
    <div
      className={classNames(
        ...
        isSticky ? styles.headerSticky : isTransparentBackground ? styles.headerFixed : styles.headerPlain
      )}
      ref={searchDrawerRef}
    >
      <div className={classNames(styles.HeaderContainer)}>
        ...
        <>
          <div className={styles.desktopNavigationMenu}>
            {navigation?.map(({ title, url }: any, currentIndex: number) => (
              <Link
                ...
                className={classNames(styles.tablink, `/${pathname?.split("/")[1]}` === url && styles.active)}
          ...
          <div className={styles.userSelectContainer}>
            <Link href="/auth/login" className={classNames(styles.login_sing_up, pathname.includes("auth") && styles.active)}>
              Log in / Sign up
            </Link>
            <div className={styles.userDataSection}></div>
      ...
      <div className={`${styles.sidenav} ${isSideNavOpen ? styles.open : ""}`}>
        ...
        <div className={styles.firstMenuContainer}>
          {navigation?.map(({ title, openNewTab, url }: any, currentIndex: number) => (
            <div key={`${title}-${currentIndex}`} className={styles.menuLinks}>
